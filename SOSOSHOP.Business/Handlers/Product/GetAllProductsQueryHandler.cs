namespace SOSOSHOP.Business.Handlers
{
	...
	public class GetAllProductsQueryHandler : IRequestHandler<GetAllProductsQuery, List<GetAllProductQueryResponse>>
    {
        #region fields
        ...
        public GetAllProductsQueryHandler(IGenericRepository<Entity.Concrete.Customer> repository,
            IMapper mapper)
        {
        ...
        public async Task<List<GetAllProductQueryResponse>> Handle(GetAllProductsQuery request, CancellationToken cancellationToken)
        {
            var response = await _repository.GetList();
            if (response != null && response.Count > 0)
            {
                return _mapper.Map<List<GetAllProductQueryResponse>>(response);
			}
